...
var myDialog =
{
    ...
    ajaxDialog: function (title, url, closeButtonText, postParams) {
        var dialog = this.modalDialog(title, '<img src="' + settings.basePath + '/img/loading.gif"></img>', closeButtonText);
        $.ajax(url, {}).done(function (data, textStatus, jqXHR) {
            $(myDialog.lastDialog).html(data);
            var divDialog = $(myDialog.lastDialog).parents('div[role="dialog"].ui-dialog');
            var height = ($(window).height()-$(divDialog).height())/2;
            var width = ($(window).width()-$(divDialog).width())/2;
            $(divDialog).css({"top":height, "left":width});
